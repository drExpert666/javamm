...
class AbstractOperationInterpreter_UnitTest {
    ...
    @BeforeEach
    void beforeEach() {
        // создаём при помощи spy, чтобы потом отследить был ли вызов именно этого объекта
    ...
    void Should_invoke_interpretOperation_after_checkForTerminate() {
        ...
        // проверка при помощи Мокито и spy, что вызывался метод именно через .interpretOperation(operation)
